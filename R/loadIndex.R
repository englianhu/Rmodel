loadIndex <- function(country, year, mbase, dbaseData = "FT", homeavd = TRUE,
  ...
  templist <- list(mbase = substitute(mbase), dbaseData =
                     dbaseData, homeavd = homeavd, basic = basic, inflated = inflated)
  tempcall <- as.call(c(expression(loadIndex), templist)); rm(templist)
  if (is.null(country)|is.null(year)) stop("enter country name and year to load index")
  if (is.null(mbase)) stop("enter soccer matches data")
  # --------------------------------------------------------------
  if(basic == TRUE){
  ...
  if(homeavd == FALSE) { lambda1 <- (1 - prob) * matchdata$hmat * matchdata$awdf
  } else { lambda1 <- (1 - prob) * matchdata$hmat * matchdata$awdf * matchdata$home }
  ...
  if(matrix == TRUE) {
    ...
    MXTtable <- diagdraw(mxt); rm(mxt, nl)
  } else { MXTtable <- "No" }
  ...
  options(warn = 0); class(result) <- 'loadIndex'
  result }
