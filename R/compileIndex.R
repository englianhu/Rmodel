compileIndex <- function (l1=FTHG~1, l2=FTAG~1, l1l2= ~c(Home,Away)+c(Away,Home), l3=~1,
                          data, maxit=300, xi=NULL, fordate=NULL, inflated=TRUE) {
  ...
  options(warn = -1)
  templist <- list(l1 = l1, l2 = l2, l1l2 = l1l2, l3 = l3, data = 
                     substitute(data), maxit = maxit, xi = xi, 
                   fordate = fordate, inflated = inflated)
  ...
  { formula1.terms <- paste(formula1.terms,
                            paste('internal.data1$noncommon*',
                                  templ1[k1], sep = ''), sep = '+')
  } else { formula1.terms <- paste(formula1.terms,
  ...
  if (checkvar1){formula1.terms<-paste(formula1.terms,
  ...
  if(!is.null(fordate)) { 
    if(!is.null(xi)) {
      if(as.Date(fordate) >= max(data$Date)) { #if(as.POSIXct(fordate) >= max(data$Date))           
        weightdata2 <- exp(-abs(xi) * as.numeric(difftime(fordate, 
  ...
  while ((difllike > pres) && (i <= maxit)) {
    ...
    if(inflated == TRUE) {
      ...
      m <- glm(formula1, family = poisson, data = data1, weights =
    ...
    names(beta) <- newnamesbeta(beta); lambda <- fitted(m)
    lambda1 <- lambda[1:n]; lambda2 <- lambda[(n + 1):(2 * n)] }
  ...
  Effects <- data.frame(Date = fordate, Home = exp((beta1[1]-beta2[1])),
  ...
  {  y <- paste(c(substring(names(x)[1:(teamn - 1)],11), teamn))
     ...
     x <- x[sort(names(x))]; z <- data.frame(Date = fordate, t(exp(x)))
  ...
  result <- list(coefficients = unlist(beta.frame), rating = beta.frame,
