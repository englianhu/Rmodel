...
# -----------------------------------------------------------
  ...
  cup.tab = lapply(cup.res, function(x) { readHTMLTable(htmlParse(getURL(x)))})
  rm(cup.res)
  ...
  for(i in seq(cup.tab)) {
    for(j in seq(cup.tab[[i]])) {
      ...
      cup.tab[[i]][[j]] = data.frame(cup.tab[[i]][[j]][1:2],matchID=rev(1:nrow(cup.tab[[i]][[j]])),
                                     cup.tab[[i]][[j]][6])
    }
  };rm(i, j)
...
# -------------------------------------------------------------------------------------
  ...
  res = list(Table = tab2, leagueID = leagueID, teamID = team, URL = sid, lnk=lnk, Rounds = lcRnds)
  options(warn = 0)
  return(res) }
