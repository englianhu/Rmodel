...
#' # this will stop
#' \dontrun{
#' compileIndex2(FTHG ~ 1, FTAG ~ 1, ~c(Home, Away) + c(Away, Home),
...
#' \dontrun{
#' em2008 = compileIndex2(FTHG~1, FTAG~1, ~c(Home, Away) + c(Away, Home), data = eng2008f) }
#' \dontrun{
#' em2009 = compileIndex2(FTHG~1, FTAG~1, ~c(Home, Away) + c(Away, Home), data = eng2009f) }
#' \dontrun{
#' em2010 = compileIndex2(FTHG~1, FTAG~1, ~c(Home, Away) + c(Away, Home), data = eng2010f) }
#' \dontrun{
#' em2011 = compileIndex2(FTHG~1, FTAG~1, ~c(Home, Away) + c(Away, Home), data = eng2011f) }
#' \dontrun{
#' em2012 = compileIndex2(FTHG~1, FTAG~1, ~c(Home, Away) + c(Away, Home), data = eng2012f) }
#' \dontrun{
#' em2013 = compileIndex2(FTHG~1, FTAG~1, ~c(Home, Away) + c(Away, Home), data = eng2013f) }
#‘
...
#' @export
#'
compileIndex2 <- function (
  l1 = FTHG ~ 1, l2 = FTAG ~ 1, l1l2 = ~c(Home, Away) + c(Away, Home),
  l3 = ~1, data, maxit = 300, xi = NULL, fordate = NULL, fun = "glm",
  inflated = TRUE) {
  ...
  templist <- list(
    ...
    maxit = maxit, xi = xi, fordate = fordate, fun = fun,
    inflated = inflated)
  ...
  if (as.character(l1[3]) == '.') {
    l1 <- formula(paste(
      as.character(l1[2]), paste(names(data1), '', collapse = '+',
                                 sep = ''), sep = '~'))
  }
  if (as.character(l2[3]) == '.') {
    l2 <- formula(paste(
      as.character(l2[2]), paste(names(data1), '', collapse = '+',
                                 sep = ''), sep = '~'))
  }
  ...
  formula2 <- formula(paste('internal.data2$y3~',
                            as.character(l3[2]), sep = ''))
    ...
    if(!is.null(xi)) {
      if(as.POSIXct(fordate) >= max(data$Date)) {
        ...
        weightdata2 <- exp(-abs(xi) * as.numeric(
          difftime(fordate, data$KODate, units = 'days')))
  ...
  if(inflated == TRUE) {
    ...
    if("glm" %in% fun){
      ...
      lambda <- fitted(speedglm::speedglm(
        formula1, family = poisson(), data = data1,
    ...
    if(inflated == TRUE) {
      ...
      if ("glm" %in% fun) {
        ...
        m0 <- glm(formula2, family = poisson, data = data2,
                  weights = (internal.data2$v1 * weightdata2),
                  maxit = 100)

      } else if ("speedglm" %in% fun) {

        m0 <- speedglm::speedglm(
          formula2, family = poisson(), data = data2,
          weights = (internal.data2$v1 * weightdata2), maxit = 100)
      }
      ...
      if ("glm" %in% fun) {
        ...
        m <- speedglm(formula1, family = poisson(), data = data1,
                      weights = (internal.data1$v1 * weightdata1),
                      maxit = 100)
      }
      ...
      if ("glm" %in% fun) {
        ...
        m0 <- speedglm::speedglm(formula2, family = poisson(),
                                 data = data2, weights = weightdata2)
        beta3 <- m0$coef
        lambda3 <- fitted(m0, data = data2)
      }
      internal.data1$y1y2 <- c(x1, x2)
  ...
  Effects <- data.frame(KODate = fordate,
  ...
  result <- list(
    coefficients = unlist(beta.frame), data = data,
    rating = beta.frame, fun = fun,
    ...
    diagonal.dist = dilabel, loglikelihood = loglike[1:i],
    call = tempcall)
